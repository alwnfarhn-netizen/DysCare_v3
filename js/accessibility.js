...
function loadAccessibilitySettings() {
    ...
    const saved = localStorage.getItem('dyscareAccessibility');
    if (saved) {
        try {
            const data = JSON.parse(saved);
            fontActive    = !!data.fontActive;
            sizeLevel     = data.sizeLevel || 0;
            contrastLevel = data.contrastLevel || 0;
            brightLevel   = data.brightLevel || 0;
        } catch(e) {}
    }

    function saveSettings() {
        localStorage.setItem('dyscareAccessibility', JSON.stringify({
            fontActive, sizeLevel, contrastLevel, brightLevel
        }));
    }
