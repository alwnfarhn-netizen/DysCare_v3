...
function navigateTo(screenId) {
    ...
    // Auto-show info popup saat masuk layar tertentu
    // Memberi panduan langsung kepada guru/orang tua/anak tentang cara pakai modul
    const autoInfoMap = {
    ...
    if (autoInfoMap[screenId]) {
        ...
        // Cek preferensi pengguna apakah popup ini sudah disembunyikan
        if (localStorage.getItem(`hide_info_${infoKey}`) !== 'true') {
            setTimeout(() => showInfo(infoKey, true), 400); // true = dipanggil secara otomatis
        }
    }
...
const INFO_CONTENT = {
    selamatdatang: {
        title: "👋 Selamat Datang di DYSCARE",
        body: `
            <p><strong>DYSCARE</strong> adalah media pembelajaran membaca permulaan untuk anak disleksia dengan pendekatan multisensori VAKT.</p>
            ...
            <p><strong>Untuk Guru dan Orang Tua:</strong></p>
            <ul class="list-disc pl-4 space-y-1">
                <li>Tekan ikon ♿ di pojok kanan atas untuk pengaturan aksesibilitas (font disleksia, ukuran teks, kontras)</li>
                <li>Tekan ikon ℹ di pojok kanan atas untuk panduan lengkap</li>
            </ul>
            <p class="text-xs text-gray-400 mt-2">Konten otomatis disesuaikan dengan level kemampuan anak. Gunakan Google Chrome untuk pengalaman terbaik.</p>
        `
    },
    membaca: {
        title: "📖 Modul Membaca",
...
// Variabel global untuk melacak popup info mana yang sedang terbuka
let currentInfoKey = null;

function showInfo(key, isAuto = false) {
    ...
    const footer = document.getElementById('info-detail-footer');
    const checkbox = document.getElementById('info-dont-show-checkbox');
    
    // Tampilkan checkbox "Jangan tampilkan lagi" HANYA JIKA popup ini muncul secara otomatis
    if (isAuto) {
...
function closeInfoModal() {
    ...
    // Jika checkbox sedang ditampilkan dan dicentang
    if (!footer.classList.contains('hidden') && checkbox.checked && currentInfoKey) {
        localStorage.setItem(`hide_info_${currentInfoKey}`, 'true');
    }
